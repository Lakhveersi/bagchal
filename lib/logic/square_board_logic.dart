...
class SquareBoardLogic {
  static List<List<Point>> initializeBoard() {
    final board = List.generate(5, (x) => List.generate(5, (y) => Point(x: x, y: y, adjacentPoints: [])));
  ...
  static List<Point> _calculateAdjacentPoints(int x, int y, List<List<Point>> board) {
  ...
  static List<Point> getValidMoves(Point piece, List<List<Point>> board) {
    ...
    for (final adjacent in piece.adjacentPoints) {
      if (adjacent.type == PieceType.empty) moves.add(adjacent);
    }
      ...
      for (final adjacent in piece.adjacentPoints) {
        if (adjacent.type == PieceType.goat) {
          ...
          if (jumpX >= 0 && jumpX < 5 && jumpY >= 0 && jumpY < 5) {
            ...
            if (landing.type == PieceType.empty && adjacent.adjacentPoints.contains(landing)) {
              moves.add(landing);
            }
          }
        }
  ...
  static MoveResult executeMove(Point from, Point to, List<List<Point>> board) {
    ...
    final bool isJump = (to.x - from.x).abs() == 2 || (to.y - from.y).abs() == 2;
    to.type = from.type;
    from.type = PieceType.empty;
    if (isJump) {
      final midX = (to.x + from.x) ~/ 2;
      final midY = (to.y + from.y) ~/ 2;
      final mid = board[midX][midY];
      if (mid.type == PieceType.goat) {
        mid.type = PieceType.empty;
